 ...
 * - 사용방법
   ...
   - 쓸 때 체크할 것(문제마다 요구하는게 조금씩 달라지기때문)
   1. 세그트리 min/max/sum
   2. 세그트리 초기화
   3. 세그트리 리턴 초기값
   4. hld_query 초기값, u와 v가 lca에서 만났을 때(u == v 일 때) 마지막 쿼리해줘야하는지
   5. u와 v가 같은 체인이 됐을 때 쿼리날릴 때 (u, v] 로 하기위해 +1 해줘야하는지)
   6. 에지에 가중치가 있는지
   7. 그래프 인접리스트 pair인지 int 인지
   8. addEdge 인자 개수와 push_back 인자
*/
struct HLD {
    struct Segtree {
        vector<int> tree;
